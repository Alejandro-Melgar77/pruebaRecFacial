...
# ------------------------
# 🔥 OCR - Serializers para Placas - TEMPORALMENTE COMENTADOS
# ------------------------
...
from .models import (
    ...
    Notification,
    MaintenanceRequest,
    VehiclePlate,          # 👈 DESCOMENTADO
    VehicleAccessLog       # 👈 DESCOMENTADO
)
...
# ------------------------
# 🔥 OCR - Serializers para Placas - DESCOMENTADOS
# ------------------------
...
class VehiclePlateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = VehiclePlate
        fields = [
            'id', 'plate_number', 'vehicle', 'unit', 'owner', 
...
class VehicleAccessLogSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            'id', 'plate_number', 'vehicle_image', 'confidence_score',
            'is_authorized', 'access_granted', 'access_type', 
            'timestamp', 'camera_location', 'notes'
        ]

class OCRPlateRecognitionSerializer(serializers.Serializer):
    image = serializers.CharField(required=True)  # Base64 image
